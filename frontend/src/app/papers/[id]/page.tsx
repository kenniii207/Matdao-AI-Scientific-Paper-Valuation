...
export default function PaperDetailPage({ params }: PageProps) {
  ...
  const [mockResult, setMockResult] = useState({
    doi,
    total_score: 60,
    grade: 'D' as 'A' | 'B' | 'C' | 'D' | 'F',
    integrity_gate_triggered: false,
    confidence_tier: 'AUTOMATED_60',
    dimensions: [
      { dimension_id: 2, raw_score: 3.0, automated: true, origin_snippet: 'Awaiting live API enrichment.' },
      { dimension_id: 9, raw_score: 3.0, automated: true, origin_snippet: 'Awaiting Crossref integrity signal.' },
      { dimension_id: 3, raw_score: 0.0, automated: false, origin_snippet: 'NULL. Requires expert review.' },
  ...
  useEffect(() => {
    const fetchResult = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/api/scoring/${encodeURIComponent(doi)}`);
        if (!res.ok) return;
        const data = await res.json();
        if (data.status === 'no_score_available') return;
        setMockResult({
          doi: data.doi,
          total_score: data.total_score,
          grade: data.grade,
          integrity_gate_triggered: data.integrity_gate_triggered,
    ...
    <main className="min-h-screen px-6 py-12 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-400 text-xs font-semibold flex items-center gap-2 animate-fade-up">
          ...
          ESTIMATED CONFIDENCE: {mockResult.confidence_tier.replace('_', ' ')}
        </div>
      </div>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex gap-4">
              <Link href={`/review/${encodeURIComponent(doi)}`} className="inline-flex items-center gap-1 text-slate-400 hover:text-indigo-300 text-sm">
                Open Expert Review Form <ExternalLink className="w-3 h-3" />
              </Link>
            </div>
          </div>
      ...
      <div className={`integrity-indicator mb-8 animate-fade-up animate-delay-1 ${
        ...
        <div>
          <p className="font-semibold text-slate-200">
            Integrity Gate: {mockResult.integrity_gate_triggered ? 'FAILED' : 'PASSED'}
          </p>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-semibold tabular-nums ${isMissing ? 'text-slate-500' : ''}`}>
                      {isMissing ? (isLoading ? 'Loading...' : 'Pending Data') : `${score.toFixed(1)}/5`}
                    </span>
                  </div>
